...
<script>
  ...
  export default {
    ...
    async created () {
      const response = await fetch('https://raw.githubusercontent.com/shanraisshan/EmojiCodeSheet/master/json/string/People.json');
      const {peoples: {people}} = await response.json();
      this.emojis = people;
    },
    methods: {
      onEmojiClick (emoji) {
